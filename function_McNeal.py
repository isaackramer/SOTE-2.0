"""Function for running SOTE using the McNeal Model"""
...
def McNeal(soils,
    ...
    eq = class_McNeal.McNeal(Cirr=Cirr,
                             Eirr=Eirr,
                             Crain=0.0,
                             Erain=0.0,
                             C_init=20.0,
    ...
    for tim in time_array[1:]:
        ...
        # net change in water content + salinity and sodicity of water
        ...
        Krel_results = np.concatenate((Krel_results, np.array([eq.Ksat])/eq.soil_parms['Ks']))

    # save output
    results_avg = pd.DataFrame(results_avg,
                               columns = ['time', 'water', 'salinity',
                                          'sodicity','hyd_cond'])
    results_avg.to_csv('output_csvs/deg_hydrus.csv')
    ends = np.array([s, q/w, E, eq.Ksat/eq.soil_parms['Ks']])
    np.savetxt('./output_csvs/deg_hydrus_ends.csv', ends, delimiter=",")
    np.savetxt('output_csvs/deg_hydrus_Ks.csv',
               Krel_results,
               delimiter=",")
